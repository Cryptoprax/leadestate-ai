...
export function LeadWizard({ action, lead, error }: { action: (form: FormData) => void | Promise<void>; lead?: LeadRecord; error?: string }) {
  ...
  return <form action={action} className="rounded-3xl border border-white/[0.08] bg-[#0a131d] p-6 sm:p-8">
    {lead && <input type="hidden" name="version" value={lead.version} />}
    <p className="text-xs uppercase tracking-widest text-cyan-300">Step {step} of 6</p>
    ...
    <div className={step === 4 ? "mt-7" : "hidden"}><div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm text-slate-500">Existing property selector · ready for AI matching<input type="hidden" name="propertyInterestIds" value="" /></div></div>
    ...
    <div className={step === 6 ? "mt-7" : "hidden"}><h2 className="text-2xl font-semibold">Review lead</h2><p className="mt-2 text-sm text-slate-500">Review the information before creating this intelligent lead workspace.</p></div>
    <div className="mt-8 flex justify-between border-t border-white/[0.07] pt-5"><Button type="button" variant="ghost" disabled={step === 1} onClick={() => setStep((current) => Math.max(1, current - 1))}>Back</Button>{step < 6 ? <Button type="button" onClick={() => setStep((current) => Math.min(6, current + 1))}>Continue</Button> : <Submit edit={Boolean(lead)} />}</div>
  </form>;
}
function Select({ name, label, items, value }: { name: string; label: string; items: readonly { code: string; label: { default: string } }[]; value: string }) { return <label className="text-sm">{label}<select name={name} defaultValue={value} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-[#0a131d] px-3">{items.map((item) => <option key={item.code} value={item.code}>{item.label.default}</option>)}</select></label>; }
