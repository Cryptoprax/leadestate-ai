...
export function DealForm({ action, deal, error }: { action: (form: FormData) => void | Promise<void>; deal?: DealRecord; error?: string }) {
  return <form action={action} className="grid gap-4 rounded-3xl border border-white/[0.08] bg-[#0a131d] p-6 sm:grid-cols-2">
    ...
    <label className="text-sm">Stage<select name="stageId" defaultValue={deal?.stageId ?? "new_lead"} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-[#0a131d] px-3">{pipelineStages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select></label>
