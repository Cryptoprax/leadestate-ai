...
async snapshot():Promise<OrganizationSnapshot>{const c=await this.context();const[profile,members,invitations,roles,departments,teams,activity]=await Promise.all([c.repository.profile(),c.repository.members(),c.repository.invitations(),c.repository.roles(),c.repository.departments(),c.repository.teams(),c.repository.activity()]);return{profile,members,invitations,roles,departments,teams,activity,canManage:["organization_owner","organization_admin"].includes(c.role),isOwner:c.role==="organization_owner"}}
