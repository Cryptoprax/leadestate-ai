...
async snapshot():Promise<OrganizationSnapshot>{const c=await this.context();const[profile,members,invitations,roles,activity]=await Promise.all([c.repository.profile(),c.repository.members(),c.repository.invitations(),c.repository.roles(),c.repository.activity()]);return{profile,members,invitations,roles,activity,canManage:["organization_owner","organization_admin"].includes(c.role),isOwner:c.role==="organization_owner"}}
