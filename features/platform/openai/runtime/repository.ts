...
export class WorkforceConversationRepository {
  ...
  async snapshot(employee: AIEmployeeCode, query = ""): Promise<ConversationSnapshot> {
    ...
    const { data, error } = await this.context.client.from("ai_workforce_messages").select("id,conversation_id,role,content,model,input_tokens,output_tokens,cost_estimate,created_at").eq("organization_id", this.context.organizationId).eq("workspace_id", this.context.workspaceId).in("conversation_id", ids).order("created_at");
    if (error) throw error;
  ...
  async append(input: { conversationId: string; role: "user" | "assistant"; content: string; model?: string; usage?: TokenUsage; cost?: CostEstimate }) {
    const { data: auth } = await this.context.client.auth.getUser();
    if (!auth.user) throw new Error("Authentication required.");
    const { error } = await this.context.client.from("ai_workforce_messages").insert({ organization_id: this.context.organizationId, workspace_id: this.context.workspaceId, conversation_id: input.conversationId, role: input.role, content: input.content, model: input.model ?? null, input_tokens: input.usage?.promptTokens ?? 0, output_tokens: input.usage?.completionTokens ?? 0, cost_estimate: input.cost?.totalUsd ?? 0, recommendation_only: true, created_by: auth.user.id });
    if (error) throw error;
