...
export function LandingRoiCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const hoursSaved = leads * 0.28;
    const aiCostSavings = hoursSaved * 10;
    return {
      revenueIncrease,
      hoursSaved,
      aiCostSavings,
      pipelineGrowth: currentDeals
        ? (improvedDeals / currentDeals - 1) * 100
        : 0,
    };
  }, [leads, conversion, revenue]);

  return (
    <div className="grid overflow-hidden rounded-[2rem] border border-vds-accent-border bg-vds-surface shadow-2xl lg:grid-cols-[.9fr_1.1fr]">
      <div className="p-6 sm:p-8">
        <h3 className="text-xl font-semibold">
          Model your monthly opportunity
        </h3>
        <p className="mt-2 text-sm leading-6 text-vds-muted">
          Use your own assumptions. Estimates are directional—not guarantees or
          financial advice.
        </p>
        <div className="mt-7 grid gap-5">
          {(
            [
              ...
              [`Revenue per deal (${currency})`, toLocal(revenue), (value: number) => setRevenue(toUsd(value)), 0, toLocal(1200000)],
            ] as const
            ...
            <label className="grid gap-2 text-sm font-medium" key={label}>
              ...
              <input
                aria-label={label}
                className="h-12 rounded-xl border border-vds-border bg-vds-input px-4 tabular-nums"
                type="number"
                min={min}
                max={max}
                value={value}
      ...
      <div
        aria-live="polite"
        className="relative border-t border-vds-border bg-vds-primary-soft p-6 sm:p-8 lg:border-l lg:border-t-0"
      >
        <TrendingUp className="size-7 text-vds-primary" aria-hidden="true" />
        <p className="mt-5 text-xs font-semibold uppercase tracking-[.18em] text-vds-primary">
          Directional impact
        </p>
        <dl className="mt-5 grid gap-5 sm:grid-cols-2">
          {[
            ["Revenue increase", format(result.revenueIncrease)],
            ["Hours saved", `${result.hoursSaved.toFixed(0)} / month`],
            ["AI cost savings", format(result.aiCostSavings)],
            ["Pipeline growth", `${result.pipelineGrowth.toFixed(0)}%`],
          ].map(([label, value]) => (
            <div
        ...
        <ButtonLink href="/roi-calculator" variant="outline" className="mt-7">
