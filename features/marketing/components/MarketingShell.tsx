...
function MarketingFooter() {
  const groups = [
    ...
    ["Social", [["LinkedIn", "/contact?intent=social&channel=linkedin"], ["X", "/contact?intent=social&channel=x"]]],
  ] as const;
  return <footer className="border-t border-vds-border bg-vds-surface/30"><div className="mx-auto grid max-w-[90rem] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[1.2fr_3fr]"><div><BrandLogo size="md"/><p className="mt-4 max-w-xs text-sm leading-6 text-vds-muted">The operating system for AI-powered businesses.</p><p className="mt-6 text-xs text-vds-subtle">Built with governance at the core.</p><form action={captureLeadAction} className="mt-8 max-w-sm"><input type="hidden" name="kind" value="newsletter"/><label className="text-sm font-semibold" htmlFor="footer-email">Enterprise launch notes</label><div className="mt-3 flex gap-2"><input id="footer-email" name="email" type="email" required maxLength={254} placeholder="Business email" className="min-w-0 flex-1 rounded-xl border border-vds-border bg-vds-input px-3 text-sm"/><Button type="submit" size="sm">Subscribe</Button></div><p className="mt-2 text-xs text-vds-subtle">Product updates. No unsolicited messages.</p></form></div><div className="grid grid-cols-2 gap-8 sm:grid-cols-3 xl:grid-cols-7">{groups.map(([title, links]) => <nav key={title} aria-label={title}><h2 className="text-sm font-semibold">{title}</h2><ul className="mt-4 space-y-3">{links.map(([label, href]) => <li key={href}><Link href={href} className="vds-focus rounded text-sm text-vds-muted hover:text-vds-foreground">{label}</Link></li>)}</ul></nav>)}</div></div><div className="border-t border-vds-border px-5 py-5 text-center text-xs text-vds-subtle">© {new Date().getUTCFullYear()} Vayon. Enterprise AI, under human control.</div></footer>;
}
