...
const schema = z.object({ kind: z.enum(["demo", "trial", "sales", "newsletter"]), name: z.string().trim().max(100).optional(), email: z.string().trim().email().max(254), company: z.string().trim().max(160).optional(), message: z.string().trim().max(2000).optional(), plan: z.enum(["starter", "professional", "enterprise"]).optional(), website: z.string().max(0).optional() });
