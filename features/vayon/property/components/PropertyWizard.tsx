...
export function PropertyWizard({ action, property, error }: { action: (form: FormData) => void | Promise<void>; property?: PropertyRecord; error?: string }) {
  ...
  return <form ref={formRef} action={action} onInput={saveLocalDraft} className="rounded-3xl border border-vds-border/[0.08] bg-[var(--vds-color-surface)] p-5 shadow-2xl shadow-vds-shadow sm:p-8">
    ...
    <div className="mt-5 grid grid-cols-10 gap-1" aria-label="Property form progress">{sections.map((section, index) => <Button variant="control" key={section} type="button" title={section} aria-label={`Open ${section}`} onClick={() => setStep(index + 1)} className={`focus-ring h-1.5 rounded-full transition ${index + 1 <= step ? "bg-vds-primary" : "bg-vds-surface/[0.07]"}`} />)}</div>
