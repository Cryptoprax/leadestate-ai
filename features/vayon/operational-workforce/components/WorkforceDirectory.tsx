...
export const WorkforceDirectory = memo(function WorkforceDirectory({ items }: { items: readonly WorkforceEmployee[] }) {
  ...
  return <section aria-labelledby="directory-title">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-vds-primary">Digital employee directory</p><h2 id="directory-title" className="mt-2 text-2xl font-semibold">Your AI team</h2></div><div className="flex gap-2 overflow-x-auto" aria-label="Filter AI employees by department">{departments.map((value) => <Button variant="control" type="button" aria-pressed={department === value} onClick={() => setDepartment(value)} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs ${department === value ? "border-vds-accent-border bg-vds-primary-soft text-vds-primary" : "border-vds-border text-vds-muted"}`} key={value}>{value}</Button>)}</div></div>
    ...
    {!visible.length && <div className="mt-5 rounded-3xl border border-dashed border-vds-border p-12 text-center text-sm text-vds-muted">No configured AI employees match this department.</div>}
  </section>;
});
