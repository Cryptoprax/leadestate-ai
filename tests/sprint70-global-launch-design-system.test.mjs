...
test("VDS owns the permanent enterprise palette and expanded tokens", () => {
  ...
  for (const color of ["#08090b", "#101114", "#181a1f", "#22252b", "#14b86a", "#37d5f2", "#16c784", "#f7b731", "#f05252", "#ffffff", "#b7bcc8", "#2b2f36", "#8b93a4"]) assert.match(css, new RegExp(color, "i"));
  const tokens = read("features/platform/design-system/tokens/tokens.ts");
  for (const contract of ["typography", "spacing", "radius", "elevation", "shadows", "motion", "breakpoints", "zIndex"]) assert.match(tokens, new RegExp(contract));
});
...
test("launch homepage communicates the operating system positioning and complete experience", () => {
  ...
  for (const item of ["The AI operating system", "AI Lead Qualification", "Marketing Automation", "Real Estate CRM", "Real Estate AI Employees", "AI Voice Agents", "Property Intelligence", "Sales Pipeline", "Secure Workspaces", "Lead arrives", "AI qualifies", "Approval", "Start Free Trial", "Book Demo", "Built for Every Real Estate Business"]) assert.match(source, new RegExp(item, "i"));
...
test("global navigation and footer expose enterprise information architecture", () => {
  ...
  for (const item of ["Platform", "Solutions", "AI Workforce", "Pricing", "Real Estate", "Resources", "Developers", "Documentation", "Login", "Start Free Trial", "Company", "Legal", "Trust Center"]) assert.match(shell, new RegExp(item));
...
test("motion is isolated, accessible, and reduced-motion aware", () => {
  ...
  assert.match(motion, /framer-motion/); assert.match(motion, /useReducedMotion/); assert.match(motion, /viewport=\{\{ once: true/);
...
test("authentication receives premium split presentation without changing actions", () => {
  ...
  assert.match(auth, /lg:grid-cols-2/); assert.match(auth, /Enterprise security by design/); assert.match(auth, /Workspace and tenant isolation/);
  const actions = read("features/authentication/actions/auth.actions.ts");
  assert.match(actions, /googleLoginAction/); assert.match(actions, /loginAction/);
});

test("Sprint 70 changes no database schema or provider implementation", () => {
  assert.equal(existsSync("docs/SPRINT_70_GLOBAL_LAUNCH_DESIGN_SYSTEM.md"), true);
  const packageJson = read("package.json"); assert.match(packageJson, /framer-motion/);
});
